...
# Příprava labelu
df = df.withColumn(
    "label", when(col("ArrDelay") > 0, 1).otherwise(0).cast(DoubleType())
)
...
indexers = [
    StringIndexer(
        inputCol=column, outputCol=column + "_index", handleInvalid="skip"
    ).fit(df)
...
assembler = VectorAssembler(
    inputCols=[
        col + "_index" if col in ["UniqueCarrier", "Origin", "Dest"] else col
...
# Trenál modelu logistické regrese
...
# Pokud accuracy < 0.55, tryne se jiný klasifikátor (random forest accuracy by měla být vyšší)
if accuracy < 0.55:
