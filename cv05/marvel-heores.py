...
connections = graph.withColumn("id", split(col("value"), " ")[0]) \
                   .withColumn("connections", split(col("value"), " ").alias("connections"))

# Calc počtu spojení
connections_count = connections.withColumn("connections_count", size(col("connections")) - 1)

# Load jmen superhrdinů
names = spark.read.option("sep", " ").csv("/files/data/marvel-names.txt").withColumnRenamed("_c0", "id").withColumnRenamed("_c1", "name")
...
# filter na minimum
most_obscure_superheroes = superheroes.filter(col("connections_count") == min_connections)
